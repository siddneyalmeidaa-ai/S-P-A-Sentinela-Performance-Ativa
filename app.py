...
# --- 2. BANCO DE DATA (QUANTUM MEMORY) COM MAPA DE PAUSAS COMPLETO ---
if 'db' not in st.session_state:
    st.session_state.db = {
        "OPERAÇÃO": {
            "ANA (PERFORMANCE)": {
                "VALOR": 45800.0, "PROJ": 91600.0, "STATUS": "85% LIBERADO", "LEGAL": "Art. 444 CLT",
                "LOG_TIME": "06:12:00", "PROD": 92, "QTD_PAUSAS": 4, 
...
with aba2:
    st.header("👥 Auditoria de Comportamento (Anatomia das Pausas)")
    op = st.selectbox("Selecione o Operador:", list(st.session_state.db["OPERAÇÃO"].keys()), key="sel_v27")
    data = st.session_state.db["OPERAÇÃO"][op]
    
    # Linha de Tempo Total
    c1, c2, c3 = st.columns(3)
    c1.metric("Tempo Logado", data["LOG_TIME"])
    c2.metric("Qtd. de Acionamentos", f"{data['QTD_PAUSAS']}x")
    c3.metric("Tempo Total em Pausa", data["TOTAL_PAUSAS"], delta="SABOTAGEM" if data['QTD_PAUSAS'] > 10 else None, delta_color="inverse")
    
    st.divider()
    # Mapeamento Específico de Pausas
    st.subheader("🛰️ Rastreio de Pausas Individuais")
    p1, p2, p3, p4 = st.columns(4)
    p1.info(f"🧘 **Pausa 01**\n\n{data['P1']}")
    p2.info(f"🧘 **Pausa 02**\n\n{data['P2']}")
    p3.success(f"☕ **Lanche**\n\n{data['LANCHE']}")
    p4.warning(f"🚾 **Banheiro**\n\n{data['BANHEIRO']}")

    if "01:" in data["BANHEIRO"]:
        st.error(f"🚨 ALERTA: Operador {op} com tempo de banheiro excessivo. Possível uso de celular ou fuga de PA.")
